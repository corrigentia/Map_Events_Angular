...
export class BaseService {
  ...
  constructor(public http: HttpClient, public stringService: StringService) {}
  ...
  GetToken(model: GetTokenBindingModel): Observable<any> {
    ...
    const body =
      'email=' +
      encodeURIComponent(model.email) +
      '&username=' +
  ...
  sendRequest(
    ...
    const url =
      (environment.isMock ? environment.mockUrl : environment.apiUrl) +
      actionPath +
      (environment.isMock ? '.json' : '');
  ...
  public post(url: string, model: any): Observable<any> {
    return this.http
      .post<any>(url, model, this.requestOptions)
  ...
  public get(url: string, model?: any): Observable<any> {
    ...
    return this.http
      .get<any>(url + params, this.requestOptions)
      .pipe(delay(200), catchError(this.handleError));
  }
