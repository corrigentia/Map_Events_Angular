...
@Component({
  ...
  styleUrls: ['google-map-demo.scss'],
  standalone: true,
...
export class GoogleMapDemo {
  ...
  mapTypeChanged(event: Event) {
    this.mapTypeId = (event.target as HTMLSelectElement)
      .value as unknown as google.maps.MapTypeId;
  }
  ...
  private _getHeatmapData(offset: number, increment: number) {
    ...
    for (
      let lat = this.center.lat - offset;
      lat < this.center.lat + offset;
      lat += increment
    ) {
      for (
        let lng = this.center.lng - offset;
